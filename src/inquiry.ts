...
async function fetchInquiry(): Promise<void> {
    ...
    const INQUIRY_URL = `http://localhost:8080/v1/api/admin/inquiry?${params.toString()}`;
...
async function deleteInquiry(inquiryId:number) {
    const DELETE_INQUIRY_URL = `http://localhost:8080/v1/api/admin/inquiry/${inquiryId}`;
    const accessToken = localStorage.getItem('accessToken');
...
async function answerInquiry(inquiryId: number, answer: string): Promise<void> {
    const ANSWER_URL = `http://localhost:8080/v1/api/admin/inquiry/answer/${inquiryId}`;
    const accessToken = localStorage.getItem('accessToken');
...
function openInquiryPopup(inquiry: Inquiry): void {
    ...
    (modal.querySelector('#inquiry-at') as HTMLInputElement).value = new Date(inquiry.inquiredAt).toLocaleString('ko-KR');
