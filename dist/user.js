...
async function fetchUsers() {
    ...
    const USER_URL = `https://d19a6mzn99qmli.cloudfront.net/v1/api/admin/users?${params.toString()}`;
...
async function deleteUser(email) {
    const DELETE_URL = `https://d19a6mzn99qmli.cloudfront.net/v1/api/admin/users/${email}`;
    const accessToken = localStorage.getItem('accessToken');
    if (!accessToken) {
        console.error('인증 토큰이 없습니다.');
        return;
