...
async function fetchPaymentDetail(paymentId) {
    const REFUND_URL = `https://d19a6mzn99qmli.cloudfront.net/v1/api/admin/payments/refunds/${paymentId}`;
...
async function approveRefund(paymentId) {
    const CONFIRM_URL = `https://d19a6mzn99qmli.cloudfront.net/v1/api/admin/payments/refunds/${paymentId}/confirm`;
    const accessToken = localStorage.getItem('accessToken');
    if (!accessToken) {
...
async function rejectRefund(paymentId) {
    const REJECT_URL = `https://d19a6mzn99qmli.cloudfront.net/v1/api/admin/payments/refunds/${paymentId}/reject`;
    const accessToken = localStorage.getItem('accessToken');
    if (!accessToken) {
        console.error('인증 토큰이 없습니다.');
        window.location.href = 'login.html';
...
function renderDetails(data) {
    ...
    const refundTypeInput = document.getElementById('refund-type');
